...
import (
	...
	cloudapi "termbridge-go/internal/cloud/api"
	cloudauth "termbridge-go/internal/cloud/auth"
	cloudauthrepo "termbridge-go/internal/cloud/auth/repository"
	clouddevice "termbridge-go/internal/cloud/device"
	cloudemail "termbridge-go/internal/cloud/email"
...
func serveHTTP(ctx context.Context, cfg Config, logger *slog.Logger, options Options, cloudHandler http.Handler, stdout io.Writer) error {
	...
	go func() {
		err := runBackendServer(options, serveCtx, server, func(info httpserver.Info) {
			fmt.Fprintf(stdout, "TermBridge cloud listening on %s\n", info.Url)
			close(backendReady)
		})
