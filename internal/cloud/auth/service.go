...
import (
	...
	"termbridge-go/internal/cloud/auth/repository"
...
func (s *Service) Capabilities() Capabilities {
	...
	mail := s.sender != nil
	return Capabilities{Providers: providers, PasswordResetEnabled: mail, EmailVerificationEnabled: mail, AccountAuthEnabled: true, CloudOAuthEnabled: false}
}
...
func (s *Service) GoogleCallback(ctx context.Context, code, state string) (Result, error) {
	if err := s.requireRepository(); err != nil {
		return Result{}, err
	}
	if s.google == nil {
		return Result{}, ErrOAuthDisabled
	}
	...
	if _, err := s.repo.FindUserByEmail(ctx, googleUser.Email); err == nil {
		return Result{}, ErrEmailAlreadyUsed
	} else if !repository.IsNotFound(err) {
		return Result{}, err
	}
...
func (s *Service) IssueUserToken(ctx context.Context, userId string) (Result, error) {
	if err := s.requireRepository(); err != nil {
		return Result{}, err
	}
...
func (s *Service) UserFromClaims(ctx context.Context, claims sharedauth.Claims) (UserView, error) {
	if err := s.requireRepository(); err != nil {
		return UserView{}, err
	}
...
func (s *Service) sign(user UserView) (Result, error) {
...
func (s *Service) verifyCode(ctx context.Context, email, purpose, input string) (repository.AuthCode, error) {
	if err := s.requireRepository(); err != nil {
		return repository.AuthCode{}, ErrCodeInvalid
	}
...
func userView(user repository.User, provider string) UserView {
