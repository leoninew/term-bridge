...
func loginCookie(t *testing.T, gateway *Server) *http.Cookie {
	t.Helper()
	response := httptest.NewRecorder()
	gateway.server.Handler.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/api/gateway/login", bytes.NewBufferString(`{"username":"admin","password":"admin"}`)))
	if response.Code != http.StatusOK {
...
func waitForRoute(t *testing.T, gateway *Server, deviceID string) {
	t.Helper()
