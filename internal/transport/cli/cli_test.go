...
func TestParseRejectsRemovedBackendCommands(t *testing.T) {
	for _, command := range []string{"gateway", "agent", "web"} {
...
func TestRunHelpWritesStdoutOnly(t *testing.T) {
	...
	for _, removed := range []string{"termbridge gateway", "termbridge agent", "termbridge web", "--gateway-url", "--device-name", "--seed-session-command"} {
