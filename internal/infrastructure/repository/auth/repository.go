...
func (r *Repository) CreateEmailUser(ctx context.Context, email, passwordHash string) (User, error) {
	...
	now := time.Now().UTC()
	user := User{ID: newID(), EmailNormalized: email, DisplayName: email, Status: StatusPending}
	identity := Identity{ID: newID(), UserId: user.ID, Provider: ProviderEmail, ProviderSubject: email, PasswordHash: sql.NullString{String: passwordHash, Valid: true}}
...
func (r *Repository) CreateGoogleUser(ctx context.Context, email, sub string) (User, error) {
	email = NormalizeEmail(email)
	now := time.Now().UTC()
	user := User{ID: newID(), EmailNormalized: email, DisplayName: email, Status: StatusEnabled, EmailVerifiedAt: sql.NullTime{Time: now, Valid: true}}
	identity := Identity{ID: newID(), UserId: user.ID, Provider: ProviderGoogle, ProviderSubject: sub, OAuthEmail: sql.NullString{String: email, Valid: true}}
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return User{}, err
	}
...
func (r *Repository) SaveEmailLog(ctx context.Context, log EmailLog) error {
	...
	_, err := r.db.ExecContext(ctx, `INSERT INTO email_delivery_logs (id,email_normalized,purpose,provider_message_id,status,response_body,created_at) VALUES (?,?,?,?,?,?,?)`, newID(), NormalizeEmail(log.EmailNormalized), log.Purpose, log.ProviderMessageID, log.Status, body, r.storeTime(time.Now().UTC()))
	return err
}
