...
- `server.static_dir: ""`
- `server.listen_url: http://127.0.0.1:9030`
- `server.public_url: http://localhost:9031`
...
```text
TERMBRIDGE_SERVER__STATIC_DIR=web
TERMBRIDGE_SERVER__LISTEN_URL=http://127.0.0.1:9033
TERMBRIDGE_SERVER__PUBLIC_URL=http://127.0.0.1:9033
```
...
4. `server.static_dir` 的相对路径会按 cwd 展开；启动脚本先切换到自身目录，`web` 会解析为 zip 解压目录下的 `web/`。
5. 前后端同源部署时，浏览器 URL、API、WebSocket 使用同一后端端口，不再依赖 Vite dev server `9031`。
6. `TERMBRIDGE_SERVER__LISTEN_URL` 和 `TERMBRIDGE_SERVER__PUBLIC_URL` 同时放在 package `.env.local` 中，确保改端口时只改 `.env.local`，启动脚本不需要改。
7. `.env.local` 不写入 `TERMBRIDGE_JWT__SECRET_KEY` 等首次启动应生成的配置；JWT 生成项在 `TERMBRIDGE_ENV=local` 时写入 `configs/config.local.yaml`，设备身份生成在 runtime state 中。
8. 生成配置的代码路径按当前环境名泛化为 `configs/config.<env>.yaml`，local 只是打包启动脚本选择的环境名，不在生成逻辑里写死。
...
3. 从 `.env.local` 读取 `TERMBRIDGE_SERVER__PUBLIC_URL` 作为浏览器打开地址；端口/URL 的修改点只在 `.env.local`。
...
- 从 `.env.local` 读取 `TERMBRIDGE_SERVER__PUBLIC_URL` 作为浏览器打开地址；端口/URL 的修改点只在 `.env.local`。
...
2. 风险：复用 `configs/config.yaml` 导致默认 `server.static_dir` 为空。
   - 缓解：package 时复制 zip 内 `.env.local`，启用 `TERMBRIDGE_SERVER__STATIC_DIR=web`。
...
5. 风险：本地服务拥有运行命令能力。
   - 缓解：默认 `configs/config.yaml` 的 `server.listen_url` 仍是 `127.0.0.1:9030`；更强认证/Host/Origin 策略作为后续安全增强。
...
- 用户指出端口/URL 的修改点应该只在 `.env.local`，不应硬编码在启动脚本；实现改为 `start.cmd` / `start.sh` 加载 `.env.local`，并从 `TERMBRIDGE_SERVER__PUBLIC_URL` 打开浏览器。
