...
- `check` 默认只读；前端使用 `lint`/`format`，Go 使用 `golangci-lint fmt --diff` 和 `run`，只有 `task check -- --fix` 才启用修复。
- 保留原有 `check` 的 Go 目录范围 `./cmd/... ./internal/...`；`test` 单独扩展到 `./migrations/...`，避免改变既有 check 门禁范围。
...
- `Taskfile.yml`：新增固定工具版本、`deps`、用户级 `install`、`proto:deps`；拆分 check/test package 范围；加入 `--fix`、`--cov` 透传；统一 Air、release 和构建环境变量入口。
...
- [x] `task deps` 下载 Go/Web 依赖，并安装固定版本工具到 `bin/`；不执行 `go mod tidy`。
- [x] `task install` 仅安装用户级 `termbridge` CLI。
- [x] `task check` 保留 `./cmd/... ./internal/...` 范围并默认只读。
...
| --- | --- |
| `task --list` | PASS |
| `task --dry check` | PASS；Go 展开为 `./cmd/... ./internal/...`，前端为只读 lint/format |
...
- 未运行常驻的 `task agent`、`task cloud` 或 `task web`，因为它们会启动开发服务器，不属于本次自动化定义的必要验证。
