...
开发者执行 `task run` 并访问 `http://localhost:9030`。浏览器继续从 `web/.env.development` 读取 hybrid 配置；Vite 将 `/local-api/*` 代理到 `http://127.0.0.1:9031`，将 `/cloud-api/*` 代理到 `http://127.0.0.1:9032`，并在转发时使后端接收到 canonical API path。
